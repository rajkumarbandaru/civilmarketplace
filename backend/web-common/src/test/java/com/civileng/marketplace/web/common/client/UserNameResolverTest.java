...
@DisplayName("UserNameResolver - caching name lookup shared by booking- and payment-service")
class UserNameResolverTest {
    ...
    @BeforeEach
    void setUp() {
        resolver = new UserNameResolver(userNameClient);
    }
    ...
    class ResolveSuccess {
        ...
        void resolve_FeignSuccess_ReturnsName() {
            ...
            when(userNameClient.getUserName(42L))
        ...
        void resolve_SecondCall_UsesCache() {
            ...
            when(userNameClient.getUserName(99L))
                    .thenReturn(ResponseEntity.ok(response));
            ...
            verify(userNameClient, times(1)).getUserName(99L);
        }
    }
    ...
    class ResolveEdgeCases {
        ...
        void resolve_UserNotFound_ReturnsPlaceholder() {
            ...
            when(userNameClient.getUserName(777L))
                    .thenReturn(ResponseEntity.ok(response));
        ...
        void resolve_FeignFailure_ReturnsFallback() {
            when(userNameClient.getUserName(anyLong()))
